...
### 37.3 UI
- **Settings → Recycle Bin** (`RecycleBinTab.tsx`, super_admin only, sits just above Danger Zone) lists restore points newest-first with table breakdown, who/when, expiry countdown, **Restore** (single confirm) and **Delete forever** (2-stage "type DELETE"). Expand a batch to see its records. After a restore/purge it clears the drill-down cache + collapses the batch, and broadcasts a `DATA_RESTORED_EVENT` (`window`) that the non-React-Query hooks (`useFiles`, `useSalvedge`, `useCoordinatorTasks`, `useNotifications`) listen for to refetch — React Query hooks are revived via `invalidateQueries`.
