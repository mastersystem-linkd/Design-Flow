...
### 37.3 UI
- **Settings → Recycle Bin** (`RecycleBinTab.tsx`, super_admin only, sits just above Danger Zone) groups restore points into **per-module sections** (Tasks / Concepts / Sampling / Full Knitting / Salvedge / Coordinator Tasks / Files / Notifications), each a proper table (Item · Deleted · By · Expires · Actions). Each batch is filed under its **primary module** (`MODULE_PRIORITY` in the route — a task+files batch → Tasks) and shows the most representative record's label ("+N more" + expandable record list for multi-row batches). **Restore** (single confirm) / **Delete forever** (2-stage "type DELETE") act per restore point. The `list` route computes the module + readable per-record labels server-side and returns them inline (no separate drill-down call). After a restore/purge it clears the drill-down cache + collapses the batch, and broadcasts a `DATA_RESTORED_EVENT` (`window`) that the non-React-Query hooks (`useFiles`, `useSalvedge`, `useCoordinatorTasks`, `useNotifications`) listen for to refetch — React Query hooks are revived via `invalidateQueries`.
