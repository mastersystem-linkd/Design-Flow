...
- **Coordinators get an actionable-only feed (don't undo this):** `design_coordinator` is deliberately **excluded** from completion/status/claim notifications — those target `["admin"]`, **not** `["admin", "design_coordinator"]`. Dropped for coordinators: task/portion completed, task fully completed, claim joined/released/resized, task split, task claimed from pool, concept started/on-hold/resumed/**added** (New Concept Submitted), Stage-2 MD-review resubmit, Full Kitting Submitted, DEO "form digitized", QC pass/fail-resample/drop. Admins still receive all of them. Coordinators **KEEP** only **actionable** sends: **Full Knitting Needed** (×3 entry points), **New Sample Pending** (task-completion + Sales ERP), **held-concept >4-day** alerts (`useHeldConceptAlerts`), **Stage-4 concept final review** (`resubmitForFinalReview` — coordinators are a Stage-4 approver), plus the **server-side** ERP new-task / new-sample and the **daily overdue** cron. The recipient set is decided **at each call site** (the role-array literal). **Don't re-add `design_coordinator` to a completion/status send** — it re-introduces the noise this removed. (Edge-function senders — ERP `ext-*`, `daily-notifications` — build recipients server-side and are all on the KEEP side; no change there.)
...
- **Group / Task Source** options are a **managed dropdown** — the `task_sources` table (migration `0086`), edited at **Settings → Dropdowns → Tasks → Task Source** (shared by brief form + EditTaskDialog, via `useTaskSources`). Each source has an **`is_whatsapp`** toggle that renders the green `<WhatsAppIcon>` in the Combobox. `src/lib/whatsappGroups.ts` is now **only the fallback** the hook uses when the table is empty / pre-migration — don't edit options there. See §16.
...
- **`task_sources`** (migration `0086`) — the brief **"Group"** picker (New Brief + Edit Task). Adds an extra **`is_whatsapp`** boolean: a per-row toggle in Settings → renders the green `<WhatsAppIcon>` in the picker. Replaces the hardcoded `lib/whatsappGroups.ts` (now fallback only). (`requester_options`, migration `0083`, similarly backs the Coordinator Tasks "Requester" picker.)
- RLS on all: read = any authed; write = `is_admin_or_coordinator()`. Coordinator write access to `concept_categories` / `fabrics` / `assigned_by_options` was widened in migration `0046`.
...
- `useTaskSources({ activeOnly })` → `{ rows, options, names, isWhatsApp(name), … }`. `rows` = raw DB (Settings editor); `options` = `{ name, is_whatsapp }[]` for the picker (falls back to `WHATSAPP_GROUPS`); `isWhatsApp(name)` gates the WhatsApp icon + the "message time required" rule. `useRequesterOptions` follows the same single-list shape.
...
- **Secondary flag column:** `<LookupSection>` also takes an optional **`flagColumn`** prop (`{ label, hint?, icon? }`) that renders a per-row boolean toggle bound to `is_whatsapp` + an add-form checkbox. **Only Task Source uses it** — it's backward-compatible, so every other dropdown is unaffected. The brief form maps `useTaskSources().options` → Combobox options with `icon: o.is_whatsapp ? <WhatsAppIcon/> : undefined`.
...
## 37. Recycle Bin — recoverable deletes (migration `0087`)

Deletes used to be permanent hard `DELETE`s with no recovery. Now **every delete is snapshotted and recoverable for 30 days**, then auto-purged. This is the safety net behind both the Danger Zone and per-row deletes (a task's ⋮ → Delete, etc.).

### 37.1 Capture — a `BEFORE DELETE` trigger, not app code
- A SECURITY DEFINER trigger `fn_archive_deleted_row()` is attached `BEFORE DELETE` on **tasks, samples, concepts, salvedge_records, task_comments, notifications, task_assignments, full_kitting_details, files, task_logs, sampling_logs, coordinator_tasks**. It writes `to_jsonb(OLD)` into **`deleted_records`** (`table_name, record_id, data jsonb, deleted_by, batch_id, expires_at, restored_at`).
- This captures **every** delete — service-role (Danger Zone), client (RLS), **and `ON DELETE CASCADE` children** — with **zero changes to delete code**. Deletes still behave identically; the bin is additive. **Do not** add app-side archive logic — the trigger owns capture. (NOT attached to profiles/clients/lookups/task_counters — protected / never user-deleted.)
- All rows removed in one transaction share `batch_id = txid_current()` → one **restore point** (a "Clear all", or a task + its cascaded files/logs/assignments). `deleted_by` is `auth.uid()` for client deletes; **null for service-role/Danger-Zone deletes** (shown as "System (Danger Zone)") — wiring an accurate `deleted_by` there would need a GUC-setting delete RPC (out of scope).
- **Storage blobs** can't be archived by a row trigger directly, so the archive trigger ALSO bins the underlying blobs of a deleted row in the **same batch** (via `fn_bin_one_blob`, dedup-guarded). **Migration 0089 covers every storage column**, not just `files.storage_url`: `tasks.full_kitting_image_url`; `full_kitting_details.image_url/file_url/files[]`; `samples.photo_url/video_url/signature_url/full_kitting_image_url`; `concepts.image_url/files[]`; `salvedge_records.attachment_url`; `sampling_logs.proof_url`. So deleting a task (or sample/concept/salvedge) makes ALL its uploaded files vanish from the Files browser and recoverable on restore. (Bucket per column: `design-files` for files/concepts*/salvedge, `sample-files` for FK/sample/concept images, `proof-photos` for sampling proofs — *concepts upload to `sample-files`.) For the Files browser / Danger Zone "Delete files", the client calls `fn_bin_storage_files(jsonb)` (SECURITY DEFINER) via `trashFiles()` in **`lib/recycleFiles.ts`** — records the file **without removing the blob** (removal happens only on purge). `useFiles` hides binned files via `fn_binned_storage_paths()` (`fetchBinnedPaths()`).
- **Files browser linking (`FilesView`):** the "LINKED TO" map resolves every storage path to its source for recognition + search — task reference files, FK images (`full_kitting_details.image_url` + `tasks.full_kitting_image_url`), salvedge attachments, **sample files labelled by `uid` · party** (photo/video/signature/FK), and **concepts** (`image_url` + `files[]`, labelled `concept_code` · title). Search matches the label + all detail values (uid, code, party, designer, …). Truly orphaned blobs (no owning row) correctly show "—".

### 37.2 Restore / purge — `api/admin-recycle-bin.ts` (super_admin only)
New Vercel route, called via `callAdminApi("admin-recycle-bin", …)` through `useRecycleBin()`. Kinds: `counts`, `list` (batch summaries), `batch` (drill-down records), `restore`, `purge`.
- **Restore** re-inserts DB snapshots **parents-first** (`RESTORE_ORDER`: concepts → samples → tasks → children → salvedge/notifications/coordinator_tasks). It supplies the original PKs + generated codes (`task_code`/`uid`/`concept_code`), which the generator triggers leave untouched (they only fire on null). Generated columns (`samples.pending_qty`, `salvedge_records.pending`) are stripped. FK gaps (e.g. a task whose concept wasn't restored) null the offending nullable FK and retry; unrecoverable rows are reported as `skipped`. `__storage__` rows just get `restored_at` set (blob never left).
- **Purge** removes `__storage__` blobs first (avoid orphans) then deletes the bin rows. Daily auto-purge of expired DB snapshots runs via pg_cron (`fn_purge_expired_recycle_bin`); expired file blobs are swept opportunistically by the route on `list`/`counts`.
- **RLS:** `deleted_records` is super_admin-only; the route + triggers use SECURITY DEFINER / service-role.

### 37.3 UI
- **Settings → Recycle Bin** (`RecycleBinTab.tsx`, super_admin only, sits just above Danger Zone) groups deleted items into **per-module sections** (Tasks / Concepts / Sampling / Full Knitting / Salvedge / Coordinator Tasks / Files / Notifications). Each section is that module's own table with **module-specific columns mirroring the real list view** (Sampling → UID/Party/Fabric/Requirement; Tasks → Code/Concept/Party/Qty/Status; Files → Name/Size/Bucket; etc.) plus Deleted/By/Expires/Actions. The `list` route returns `{ sections: [{ module, columns, rows }] }` — **one row per deleted main entity** (via `LISTABLE` + `MODULE_COLUMNS` + `recordCells`, resolving party/designer names server-side); internal children (logs/assignments/comments/sampling_logs) are NOT listed but **ride along on the batch restore**. **Restore** / **Delete forever** (2-stage "type DELETE") act on the row's whole **batch** (so a task + its files/logs come back together); the confirm shows `batch_total`. After a restore/purge it clears the drill-down cache + collapses the batch, and broadcasts a `DATA_RESTORED_EVENT` (`window`) that the non-React-Query hooks (`useFiles`, `useSalvedge`, `useCoordinatorTasks`, `useNotifications`) listen for to refetch — React Query hooks are revived via `invalidateQueries`.
- **Danger Zone** now carries a "deletes are recoverable for 30 days" banner, is grouped into labelled sections (Quick clears / Delete by table / Clear everything), and its copy says items move to the Recycle Bin (no longer "cannot be undone"). The `admin-clear-data` server role check was **tightened to super_admin** to match the UI gate.

### 37.4 Hardening from the pre-deploy review (migration `0088`)
An adversarial review surfaced restore/data-loss edge cases — fixes (do NOT regress these):
- **Clear-all is now ONE transaction** via `fn_clear_all_transactional()` (service_role-only RPC) so it forms a single restore point (parents-first restorable), and it **no longer resets `task_counters`** — resetting it left the counter at 0 while restored tasks held `ORD-YYYY-0001…`, breaking the next task's `task_code`. A monotonic counter is harmless.
- **Storage dedup:** a blob is never binned twice while active (`fn_archive_deleted_row` files-branch + `fn_bin_storage_files` both skip if an active `__storage__` row already exists for that `(bucket, path)`), so a per-batch purge can't destroy a blob another batch still references. Purge also has a cross-batch guard before removing a blob.
- **Restore engine** (`admin-recycle-bin.ts`): `RESTORE_ORDER` is concepts → **tasks → samples** → children (samples.task_id → tasks); `NULLABLE_FKS` covers `tasks.concept_id`, `samples.task_id`, `coordinator_tasks.related_task_id`. A unique-violation on re-insert is only treated as "already restored" if the snapshot's **own PK** exists (else it's kept in the bin, not silently retired). After restoring `task_assignments`, the parent task's `status/qty_*/is_split` are re-asserted from the snapshot (the recalc trigger would otherwise recompute them). A binned blob is only un-binned when its paired `files` row actually restored. Restore is bounded to the active (non-expired) window; purge isn't.
- **Accepted minor limitations (documented, not bugs):** restoring a task fires `tasks_log_change` → one extra "Task created" audit row; full_kitting_details can't be FK-nulled (XOR CHECK) so it only restores with its parent; expired **file** blobs are swept opportunistically when the Recycle Bin is opened (`list`/`counts`) rather than by the daily DB cron (SQL can't call Storage).
