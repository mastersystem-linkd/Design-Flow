...
- **Storage blobs** can't be archived by a row trigger. The `files`-row branch of the trigger ALSO bins the underlying `design-files` blob as a `__storage__` row in the **same batch**, so a deleted task's files are recoverable with it. For the Files browser / Danger Zone "Delete files", the client calls `fn_bin_storage_files(jsonb)` (SECURITY DEFINER) via `trashFiles()` in **`lib/recycleFiles.ts`** — it records the file **without removing the blob** (removal happens only on purge). `useFiles` hides binned files via `fn_binned_storage_paths()` (`fetchBinnedPaths()`).
